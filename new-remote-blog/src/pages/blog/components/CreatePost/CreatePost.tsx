...
import { useAddPostMutation, useGetPostByIdQuery, useUpdatePostMutation } from '../../blog.service'
...
export default function CreatePost() {
  ...
  const [addPost, addPostResult] = useAddPostMutation() // [function, object]
  const postId = useSelector((state: RootState) => state.blog.postId)
  const { data } = useGetPostByIdQuery(postId, { skip: !postId, refetchOnMountOrArgChange: 10, pollingInterval: 1000 })
        ...
        <div className='mb-6'>
          <label
            ...
            className={classNames('mb-2 block text-sm font-medium  ', {
              ...
              'text-gray-900 dark:text-gray-300': errorForm?.publishDate === undefined
            })}
          >
            Publish Date
          </label>
          <input
            ...
            className={classNames('block w-56 rounded-lg border  p-2.5 text-sm focus:outline-none', {
              ...
              'border-gray-300 bg-gray-50 text-gray-900 focus:border-blue-500 focus:ring-blue-500':
                errorForm?.publishDate === undefined
            })}
            required
